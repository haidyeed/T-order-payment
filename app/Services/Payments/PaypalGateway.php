...
class PaypalGateway implements PaymentGatewayInterface { 
        ...
        private function buildBody($paymentData)
    {
        ...
        Log::channel('payment')->info("PAYPAL Payment Body", ['userId' => $body['userId'], 'amount' => $body['amount']]);
    ...
    private function makeTransaction($body)
    {
        ...
        Log::channel('payment')->info("PAYPAL Payment Transaction process", ['transaction_status' => $body['transactionStatus'], 'amount' => $body['amount']]);
    ...
    private function sendEmail($transaction)
    {
        try {
            ...
            Log::channel('payment')->info("PAYPAL Payment Email", ['status' => 'sended successfully']);

        } catch (\Throwable $e) {
            Log::channel('payment')->info("PAYPAL PAyment, Failed to sent Email with excpetion: " . $e->getMessage());
        }
    }
